...
async def main():
    ...
    # 初始化：预计算 + 沙箱 + agent graph
    print("正在初始化（预计算 MCP 工具 + 创建沙箱）...")
    precomputed = await precompute_agent_context()
    sandbox_backend = setup_sandbox(SANDBOX_CONFIG)
    config = {"configurable": {"thread_id": "test-thread-001", "user_id": "laoxiao"}}
    ...
    while True:
        try:
            ...
            # 流式处理并获取响应（内含中断恢复循环）
            final_response = await stream_agent_response(agent_graph, user_input)
