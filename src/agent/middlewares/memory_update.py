...
class MemoryUpdateMiddleware(AgentMiddleware):
    ...
    async def aafter_agent(
        ...
        try:
            ...
            # 5.5 写入前校验——双信任级防线
            # （loop 改造 D：防幻觉 + 安全整改 P0-3：防外部内容投毒）
            #
            # 幻觉防线（D，原有）：LLM 抽取的实体若在对话原文中找不到精确
            #   匹配（如把谐音"博士"错记成"博世"），拦截——记忆里的错误
            #   实体会被每轮读取、每轮合并，一次写错复利到所有后续对话。
            #
            # 投毒防线（P0-3，新增）：只信任**用户消息**中出现过的实体。
            #   AI 回复可能转述了 web_search 等外部内容——外部内容里的
            #   恶意实体一旦经 AI 回复满足"在对话原文出现过"的旧规则，
            #   就会获得长期记忆的永久席位（此后每轮对话都带着它）。
            #   因此：用户消息中出现过 = 高信任，可写入；仅在 AI 回复
            #   出现、用户从未提及 = 低信任（可能来自外部内容转述），
            #   拦截并记日志。
            #   代价是功能性的：用户问"推荐个供应商"、AI 回复"博世的
            #   刹车片不错"——"博世"不再进记忆。宁可漏记（下轮对话再问
            #   一次），不可错记（污染永久跟随）。
            validated_suppliers = []
            for s in suppliers:
                s_str = str(s).strip()
                if not s_str:
                    continue
                if s_str in user_message:
                    validated_suppliers.append(s_str)  # 高信任：用户亲口提过
                elif s_str in ai_summary:
                    logger.warning(
                        "MemoryUpdateMiddleware: 拦截低信任实体 %r（仅在 AI 回复出现、"
                        "用户消息未提及——可能来自外部内容转述，不写入长期记忆）", s
                    )
                else:
                    logger.warning(
                        "MemoryUpdateMiddleware: 拦截疑似幻觉实体 %r（对话原文未出现，不写入记忆）", s
                    )
