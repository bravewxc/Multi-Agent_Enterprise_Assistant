...
from agent.env_utils import ANALYSIS_MCP_URL
...
MCP_SERVER_CONFIG = {
    ...
    "analysis": {
        # 本机自托管 AntV mcp-server-chart（启动方式见 env_utils.py ANALYSIS_MCP_URL 注释）。
        # 服务未启动时这里降级为空列表（图表类 0 个），ERP 业务不受影响。
...
CHART_TOOL_PREFIXES = ("generate_",)  # 魔搭社区 MCP 可视化+工具（26 图表/地图/图表 + 1 spreadsheet）
# 智能补货子 Agent（procurement-replenish）专属工具组
REPLENISH_TOOL_PREFIXES = ("replenish_",)
...
async def load_mcp_tools(
    ...
    try:
        ...
        print(f"[WARNING] 魔塔社区 MCP Server 不可用，跳过图表工具加载: {e}")
        analysis_tools = []
